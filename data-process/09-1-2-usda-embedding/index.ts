...
// 入力・出力ファイル
const inputFile = join(__dirname, '../09-1-1-usda-normalize/result/usda-foods.json');
...
async function loadProgress(): Promise<Progress> {
  try {
    ...
    return { processedFdcIds: [], totalTokens: 0, batchCount: 0 };
  }
}
...
async function main() {
  ...
  console.log('=== 09-1-2: USDA食品名Embedding ===\n');

  // USDAデータを読み込む
  console.log('USDAデータを読み込んでいます...');
  const usdaContent = await readFile(inputFile, 'utf-8');
  const usdaFoods: Record<string, USDAFood> = JSON.parse(usdaContent);
  
  const foodList = Object.values(usdaFoods);
  console.log(`USDA食品数: ${foodList.length}件`);

  // 進捗を読み込む
  const progress = await loadProgress();
  const processedSet = new Set(progress.processedFdcIds);
  ...
  // 未処理の食品を抽出
  const unprocessedFoods = foodList.filter(f => !processedSet.has(f.fdc_id));
  ...
  if (unprocessedFoods.length === 0) {
    ...
    for (let i = 0; i < unprocessedFoods.length; i += BATCH_SIZE) {
      ...
      // descriptionを取得
      const texts = batch.map(f => f.description);
      ...
      try {
        ...
        // 進捗を保存
        progress.processedFdcIds = Array.from(processedSet);
